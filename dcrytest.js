...
async function decryptjalkey(url) {
  ...
  const browser = await puppeteer.launch({ headless: 'new', executablePath: 'C:\Program Files\Mozilla Firefox\\firefox.exe' });

  const page = await browser.newPage();

  await page.goto(`https://encode-decode.com/${url}-encrypt-online/`);

  await page.waitForTimeout(2000); // Esperar 2 segundos antes de buscar el selector

  await page.waitForSelector('textarea[name="encryption_destinationText"]', { timeout: 60000 });
